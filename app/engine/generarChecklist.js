...
export function generarChecklist(rules, scope, modalidad, ruta_id, input) {
  ...
  if (modalidad === "EXENTA") {
    notas.push("Excepción: no corresponde licencia. Conserva evidencia y verifica obligaciones asociadas (si aplica).");
    return { items, notas, warnings };
  }

  const ed = rules.edificacion;
  const mod = ed?.modalidades?.[modalidad];
